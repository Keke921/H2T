...
from utils import config, update_config, create_logger, SetRandomSeed, save_code, save_checkpoint
...
def parse_args():
    ...
    parser.add_argument('--cfg',
                        ...
                        default='./config/cifar100/cifar100_imb001_stage2_ce_H2T.yaml',                        
                        #required=True,
                        type=str)
...
def main():
    args = parse_args()
    logger, model_dir, code_dir = create_logger(config, args.cfg)
    logger.info('\n' + pprint.pformat(args))
    logger.info('\n' + str(config))
    save_code(code_dir)
...
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.dataset == 'cifar10' or config.dataset == 'cifar100':
        ...
        #classifier = getattr(resnet_cifar, config.classifier)(config, feat_in=64, num_classes=config.num_classes, 
        ...
        classifier = getattr(ride_resnet, config.classifier)(config, feat_in=2048, num_classes=config.num_classes)

    elif config.dataset == 'places':
        model = getattr(resnet_places, config.backbone)(pretrained=True)
        classifier = getattr(ride_resnet_places, config.classifier)(config, feat_in=2048, num_classes=config.num_classes)
        block = getattr(resnet_places, 'Block')()
            ...
            with torch.no_grad():
                if config.dataset == 'places':
                    if config.H2T > 0:
                        ...
                        feat = H2T(feat1, feat2, rho =config.H2T)
                    else: 
