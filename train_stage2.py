...
from models.ride_model.ldam_drw_resnets import ride_resnet_cifar
from models.ride_model.ldam_drw_resnets import ride_resnet
from models.ride_model.ldam_drw_resnets import ride_resnet_places
...
from methods import mixup_data, mixup_criterion, H2T,LearnableWeightScaling  
from loss import *
...
def parse_args():
    ...
    parser.add_argument('--cfg',
                        ...
                        default='./config/cifar10/ResLT/imb0005/cifar10_imb0005_stage2_ResLT_fhft.yaml',                        
                        #required=True,
                        type=str)
...
def main_worker(gpu, ngpus_per_node, config, logger, model_dir):
    global best_acc1, its_ece, best_epoch
    config.gpu = gpu
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.dataset == 'cifar10' or config.dataset == 'cifar100':
        ...
        classifier = getattr(config, ride_resnet_cifar, config.classifier)(feat_in=64, num_classes=config.num_classes, 
                                                              use_norm=config.use_norm, num_experts=config.num_experts)
    elif config.dataset == 'imagenet' or config.dataset == 'ina2018':
        model = getattr(ride_resnet, config.backbone)()
        classifier = getattr(config, ride_resnet, config.classifier)(feat_in=2048, num_classes=config.num_classes)

    elif config.dataset == 'places':
        model = getattr(ride_resnet_places, config.backbone)(pretrained=True)
        classifier = getattr(config, ride_resnet_places, config.classifier)(feat_in=2048, num_classes=config.num_classes)
        block = getattr(ride_resnet_places, 'Block')()
    ...
    if config.dataset == 'cifar10':
        dataset = CIFAR10_LT(config, config.distributed, root=config.data_path, imb_factor=config.imb_factor,
                             batch_size=config.batch_size, num_works=config.workers)

    elif config.dataset == 'cifar100':
        dataset = CIFAR100_LT(config, config.distributed, root=config.data_path, imb_factor=config.imb_factor,
                              batch_size=config.batch_size, num_works=config.workers)

    elif config.dataset == 'places':
        dataset = Places_LT(config, config.distributed, root=config.data_path,
                            batch_size=config.batch_size, num_works=config.workers)

    elif config.dataset == 'imagenet':
        dataset = ImageNet_LT(config,config.distributed, root=config.data_path,
                              batch_size=config.batch_size, num_works=config.workers)

    elif config.dataset == 'ina2018':
        dataset = iNa2018(config, config.distributed, root=config.data_path,
                          batch_size=config.batch_size, num_works=config.workers)
    ...
    #criterion = LDAMLoss(cls_num_list=cls_num_list, s=1).cuda(config.gpu)
    criterion = eval(config.loss.type)(config=config, cls_num_list=cls_num_list).to('cuda')  #cls_num_list=cls_num_list,
    ...
    for epoch in range(config.num_epochs):  #start_epoch, 
        ...
        if config.dataset != 'places':
            block = None
        # train for one epoch
        train(train_loader, model, classifier, criterion, optimizer, epoch, config, logger, block)  #, lws_model

        # evaluate on validation set
        acc1, ece = validate(val_loader, model, classifier, criterion, config, logger, block) #, lws_model
        ...
        if is_best:
            its_ece = ece
            best_epoch = epoch
        logger.info('Best Prec@1: %.3f%% ECE: %.3f%% Best epoch: %d \n' % (best_acc1, its_ece, best_epoch))
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, config, logger, block=None):   #, lws_model
    ...
    for i, (images, target, meta) in enumerate(train_loader):
        if i > end_steps:
            break
        extra_info = {}
        if config.mix_fea>0:
            images2 = meta['sample_image']
        ...
        if torch.cuda.is_available():
            ...
            if config.mix_fea > 0 :
                images2 = images2.cuda(config.gpu, non_blocking=True)  
        if config.mixup is True:
            images, targets_a, targets_b, lam = mixup_data(images, target, alpha=config.alpha)
            with torch.no_grad():
                if config.dataset == 'places':
                    if config.mix_fea > 0:
                        feat1, feat2 = block(model(images)), block(model(images2))                   
                        feat = H2T(feat1, feat2, beta =config.mix_fea)
                    else: 
                        feat = block(model(images))
                else:
                    if config.mix_fea > 0:
                        #feat = model(images, images2, beta =config.beta)  
                        feat1, feat2 = model(images),model(images2)                   
                        feat = H2T(feat1, feat2, beta =config.mix_fea)                        
                    else:
                        feat = model(images)
            output = classifier(feat)
            #output = lws_model(output)
            if config.loss.add_extra_info:
                extra_info.update({"logits": classifier.module.logits}) 
            else:
                extra_info = None                
            loss = mixup_criterion(criterion, output, targets_a, targets_b, lam, extra_info=extra_info)
        else:
            # compute output
            with torch.no_grad():
                if config.dataset == 'places':
                    ...
                    if config.mix_fea > 0:
                        ...
                        feat = model(images)
                '''        
                if config.mix_fea is True:
                    feat1, feat2 = model(images),model(images2)
                    if config.dataset == 'places':
                       feat1, feat2 = block(feat1), block(feat2) 
                    feat = H2T(feat1, feat2, beta =config.beta)  
                else:
                    feat = model(images)
                    if config.dataset == 'places':
                       feat = block(model(images))
            ...
            if config.loss.add_extra_info:
                ...
                extra_info = None
            loss = criterion(output, target, extra_info=extra_info)

        if isinstance(output, list):
            output = torch.stack(output, dim=1).sum(dim=1) 
...
def validate(val_loader, model, classifier, criterion, config, logger, block=None):  #, lws_model
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.3f')
    top1 = AverageMeter('Acc@1', ':6.3f')
    top5 = AverageMeter('Acc@5', ':6.3f')
...
def adjust_learning_rate(optimizer, epoch, config):
    ...
    for idx, param_group in enumerate(optimizer.param_groups):
        if idx == 0:
            param_group['lr'] = config.lr_scheduler.lr_factor * lr
        else:
