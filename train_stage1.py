...
def parse_args():
    parser = argparse.ArgumentParser(description='RIDE training (Stage-1)')
    parser.add_argument('--cfg',
                        help='experiment configure file name',
                        default='./config/cifar100/cifar100_imb001_stage1_ce_mixup.yaml',                        
                        #required=True,
                        type=str)
...
def main_worker(gpu, ngpus_per_node, config, logger, model_dir):
    global best_acc1, best_epoch, its_ece
    config.gpu = gpu
#     start_time = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    if config.dataset == 'cifar10' or config.dataset == 'cifar100':
        ...
        classifier = getattr(resnet_cifar, config.classifier)(feat_in=64, num_classes=config.num_classes)

    elif config.dataset == 'imagenet' or config.dataset == 'ina2018':
        model = getattr(resnet, config.backbone)()
        classifier = getattr(resnet, config.classifier)(feat_in=2048, num_classes=config.num_classes)

    elif config.dataset == 'places':
        model = getattr(resnet_places, config.backbone)(pretrained=True)
        classifier = getattr(ride_resnet_places, config.classifier)(feat_in=2048, num_classes=config.num_classes)
    ...
    #criterion = LDAMLoss(cls_num_list=dataset.cls_num_list, s=1).cuda(config.gpu)
    # get function handles of loss    
    criterion = eval(config.loss.type)(config=config, cls_num_list=dataset.cls_num_list).to('cuda')
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, config, logger, block=None):
    ...
    for i, (images, target, _) in enumerate(train_loader):
        ...
        if config.mixup is True:
            ...
            if config.loss.add_extra_info:
                extra_info.update({"logits": classifier.logits })  
            else:
                extra_info = None
...
def adjust_learning_rate(optimizer, epoch, config):
    ...
    if config.lr_scheduler.type == 'cos':
        lr_min = 0
        lr_max = config.lr
        ...
        elif epoch > config.lr_scheduler.lr_step[1]:
            lr = config.lr * 0.01
        elif epoch > config.lr_scheduler.lr_step[0]:
            lr = config.lr * 0.1
        else:
            lr = config.lr
