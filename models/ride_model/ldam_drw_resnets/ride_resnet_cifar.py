...
class ResNet_s(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_experts=3, num_classes=10, reduce_dimension=False, layer2_output_dim=None, layer3_output_dim=None, use_norm=False, use_experts=None):
...
class Classifier(nn.Module):
    ...
    def forward(self, x):
        ...
        for ind in range(len(x)):
            out = F.avg_pool2d(x[ind], x[ind].size()[3])
            out = out.view(out.size(0), -1)      
            self.feat.append(out)
            out = (self.linears[ind])(out)
...
class ResClassifier(nn.Module):
      def __init__(self, config, feat_in, num_classes=10, use_norm=False, scale=1, num_experts=3):
          ...
          self.BNH = nn.BatchNorm2d(feat_in) # nc[2]
          self.BNM = nn.BatchNorm2d(feat_in) # nc[2]
          self.BNT = nn.BatchNorm2d(feat_in) # nc[2]
      ...
      def forward(self, x):
          ...
          outs = []
          for ind in range(len(x)):
              head_fs, medium_fs, tail_fs=self.BNH(x[ind]), self.BNM(x[ind]), self.BNT(x[ind]) 
              fs = torch.cat((head_fs, medium_fs, tail_fs),dim=0)
              out = self.avgpool(fs).view(fs.size(0),-1)
              out = self.linear[ind](out)      
              outs.append(out) 
          self.logits = outs    
...
def resnet32(num_classes=10, num_experts=3, use_norm=False):
    return ResNet_s(BasicBlock, [5, 5, 5], num_classes=num_classes, num_experts=num_experts, use_norm=use_norm)
